...
$$\begin{aligned}
  x_{t+1}^1 &= h\left(x_t^1, 
    (\mathbb E_t [u_\tau])_{\tau \ge t} \right) + \epsilon_{t+1}\\
  x_t^2 &= g\left(x_t^1, 
     (\mathbb E_t [u_\tau])_{\tau \ge t} \right) 
\end{aligned}$$

と表される。ただし, $\epsilon$ は予測誤差

$$\mathbb E_t [\epsilon_{t+1}] = 0$$
...
* $\mathbb E_t x_{t+1}^1 = x_{t+1}^1 + \epsilon_{t+1}$, すなわち
  $\mathbb E_{t} \epsilon_{t+1} = 0$
  
$t$期では予測できない誤差を除いて，  $t+1$期の値を決めることができる変数が先決変数

分散共分散行列 $\mathbb E_t \epsilon_{t+1}\epsilon_{t+1}^\top$ は positive semidefinite ( 決定論的な成分を含んでいてもよい $\epsilon_{t+1, i} = 0$ a.s.)
...
$$\begin{aligned}
  E
  \begin{bmatrix}
    \mathbb E_t x_{t+1}^1 \\ \mathbb E_t x_{t+1}^2
  \end{bmatrix}
  =
  A
  \begin{bmatrix}
   \mathbb E_t x_{t}^1 \\ \mathbb E_t x_{t}^2
  \end{bmatrix}
\end{aligned}$$
...
$$\begin{aligned}
x_{t+1}^{1} &= h(x_t^1) + \epsilon_{t+1}
  = Z_{1s}S_{ss}^{-1}T_{ss}Z_{1s}^{-1} x_{t}^{1} + \epsilon_{t+1}\\
x_{t}^{2} &= g(x_t^1) 
  = Z_{2s}Z_{1s}^{-1} x_{t}^{1}
\end{aligned}$$
